...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QPushButton, QLabel, QPlainTextEdit, QSpinBox, QProgressBar,
    QTabWidget, QGridLayout, QSlider, QSplitter, QLineEdit, QMessageBox
)
...
class ThreadDemoWindow(QMainWindow):
    ...
    def run_simple_task(self):
        ...
        thread.add_done_callback(lambda result: self.on_task_complete(result, "简单任务"))
        thread.start()
    ...
    def run_param_task(self):
        ...
        thread.add_done_callback(lambda result: self.on_task_complete(result, "带参数任务"))
        thread.start()
    ...
    def run_long_task(self):
        ...
        thread = QThreadWithReturn(task)
        thread.add_done_callback(lambda result: self.on_task_complete(result, "长时间任务"))
        thread.start()
    ...
    def run_error_task(self):
        ...
        thread = QThreadWithReturn(task)
        thread.add_done_callback(lambda result: self.on_task_complete(result, "错误任务"))
    ...
    def graceful_cancel_task(self):
        ...
        if hasattr(self, 'cancelable_thread') and self.cancelable_thread.running():
            ...
            if success:
                self.log_widget.add_log("优雅取消请求已发送（等待任务自行检查中断标志）", "WARNING")
            else:
    ...
    def run_timeout_task(self):
        ...
        thread = QThreadWithReturn(task)
        thread.add_done_callback(lambda result: self.on_task_complete(result, "超时任务"))
    ...
    def run_multi_return_task(self):
        ...
        thread.add_done_callback(
            lambda a, b, c: self.on_task_complete(f"多返回值: {a}, {b}, {c}", "多返回值任务")
        )
        thread.start()
    ...
    def create_thread_pool(self):
        ...
        self.log_widget.add_log(f"线程池创建成功（最大线程数: {max_workers}）", "SUCCESS")
    ...
    def submit_batch_tasks(self):
        ...
        for i in range(task_count):
            ...
            future.add_done_callback(
                lambda result, tid=i+1: self.on_task_complete(result, f"批量任务 {tid}")
            )
    ...
    def run_stress_test(self):
        ...
        def wait_all():
            ...
            self.log_widget.add_log(
                f"压力测试完成！{concurrency} 个任务耗时: {elapsed:.2f}秒",
                "SUCCESS"
            )
            pool.shutdown()
    ...
    def update_monitor(self):
        ...
        memory_usage = min(100, 30 + self.active_thread_count * 5 + random.randint(0, 10))
